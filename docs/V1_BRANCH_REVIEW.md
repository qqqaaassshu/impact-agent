...
本文档记录 `featureChange` 分支在 V1 阶段的项目优点、当前不足和后续调整方向。它用于帮助后续接手者判断下一步应该优先补哪里，而不是替代 `README.md` 或 `CLAUDE.md`。

## 当前结论

这个分支已经具备一个可运行的前端影响范围分析 Agent 基础版本：

- 可以构建本地前端仓库索引。
- 可以使用 Chroma 做语义检索。
- 可以使用 SQLite 保存索引元数据和记忆数据结构。
- 可以接入 DeepSeek 等 OpenAI-compatible chat model。
- 可以使用 Ollama `bge-m3` 做本地 embedding。
- 可以通过 WebApp 触发索引、搜索和分析。
- 可以通过流式接口展示 ReAct 分析过程。
- 可以在候选影响项中展示原因和证据。

V1 的价值在于完成“需求变更影响范围分析”的核心闭环：索引、检索、模型决策、工具调用、证据展示、人工复核。
...
这对产品可信度具有关键作用。影响分析不同于普通问答，用户需要了解 Agent 的检索路径、检索原因和检索结果，才能对报告进行有效复核。
...
这符合项目目标：输出可复核的影响范围，而不是不可验证的文件列表。
...
因此，V1 的结果应定位为“候选影响范围”，不应作为完整确定的影响闭环。

### 2. Chroma 检索质量还需要调优

当前语义检索已可使用，但仍需要进一步质量优化：
...
- 页面不知道进度。
- 用户无法明确区分当前处于扫描、embedding 还是写入 Chroma 阶段。
- 不能取消。
- 失败恢复弱。
...
原因是前者解决可用性问题，后者解决结果质量问题。两项能力稳定后，再扩展代码图、记忆和报告体验，整体收益更明确。
